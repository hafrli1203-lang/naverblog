...
### 40. OAuth 로그인 속도 개선 — 백그라운드 워밍업 + 재시도 단축 (2026-02-26)

**커밋:** `acc3384`

**수정 파일:** `frontend/src/main.js`, `backend/app.py` (2개)

**문제**: 카카오 로그인 성공 후에도 연결 속도가 느림. Render 무료 플랜 콜드 스타트(15~30초) + 프록시 재시도 대기(최대 35초)가 원인.

**수정 3건:**

1. **페이지 로드 시 백그라운드 워밍업 (`main.js`):**
   - `DOMContentLoaded`에서 `fetch(`${AUTH_BASE}/auth/me`)` 비동기 호출 (fire-and-forget)
   - 사용자가 페이지를 열면 인증 서버가 백그라운드에서 깨어남
   - 로그인 클릭 시 이미 서버 준비 완료 → 즉시 응답

2. **로그인 클릭 시 워밍업 타임아웃 단축 (`main.js`):**
   - `AbortSignal.timeout(40000)` → `AbortSignal.timeout(8000)` (40초→8초)
   - 페이지 로드 워밍업으로 이미 깨어있을 확률 높음

3. **프록시 재시도 대기 시간 단축 (`app.py`):**
   - Before: `2 * (attempt + 1) + 1` = 3+5+7+9+11 = **35초**
   - After: `min(attempt + 1, 5)` = 1+2+3+4+5 = **20초** (43% 단축)
...
|------|-----------|
| 워밍업 `/auth/me` | 0.58초 |
| 카카오 리다이렉트 `/auth/kakao` | 0.52초 |
| 토큰 교환 `/auth/token-exchange` | 0.60초 |
| **서버 측 합계** | **~1.7초** |
